      ...
      (let ((x 0))
        (loop
          ...
          (when (bullet-hit)
            ...
            (when (>= 0 (zombie-hp (elt z-list x)))
              (let ((y 0))
                (loop
                  (when (>= y (length players))(return))
                  (when (equal (playa-hit (elt players y)) (zombie-id (elt z-list x)))
                    (setf (playa-hit (elt players y)) nil)
                  )
                  (incf y)
                )
              )
              (setf z-list (remove (elt z-list x) z-list :test #'eql))
            )
          )
          (incf x)
        )
